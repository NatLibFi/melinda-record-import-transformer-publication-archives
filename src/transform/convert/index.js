...
export default ({harvestSource, urnResolverUrl}) => {
	return record => {		
		const fields = record.metadata[0]['kk:metadata'][0]['kk:field'];
		const marcRecord = new MarcRecord();

		marcRecord.leader = '01704nam a22002653i 4500';
		...
		function generateFields() {
			...
			function generate100and700() {
				...
				function generateWriters() {
					const values = getFieldValues(p => ['dc.contributor.author', 'dc.creator'].includes(p));
			...
			function generate653() {
				const values = getFieldValues(p => ['dc.subject.ysa', 'dc.subject'].includes(p));
				return values.map(value => ({
					tag: '653', ind1: '', ind2: '',
					subfields: [{code: 'a', value}]
				}));
			...
			function generate020() {
				...
				return values.map(value => {
					...
					function formatValue() {
						return value.replace(/(ISBN)|(\s+)|(\(print\))/g, '');
					}
				});
			}
			...
			function generate245() {
				...
				function generateIsAddedEntry() {
					const fields = getFields(p => ['dc.contributor.author', 'dc.author'].includes(p));
			...
			function generate490() {
				...
				function generateSubfields() {
					const isPartOfSeries = getFieldValues('dc.relation.ispartofseries');

					if (isPartOfSeries.length > 0) {
						const tail = generateTail();

						if (tail.length > 0) {
							return [
								{
									code: 'a', value: `${isPartOfSeries[0]}`
								}
							].concat(tail);
						}
					...
					function generateTail() {
						const issn = getIssnValues();
						const seriesNumber = getFieldValues(p => p === 'dc.relation.numberinseries' || p === 'dc.relation.numberofseries');

						if (issn.length > 0 && seriesNumber.length > 0) {
							return generateIssn(true).concat({code: 'v', value: seriesNumber[0]});
						}
						...
						function generateIssn(hasSeriesNumber) {
							const list = issn.reduceRight((acc, v) => {
								if (acc.length > 0) {
									return acc.concat({code: 'x', value: `${v}, `});
								}
							...
							// Reverse
							return list.reduceRight((acc, v) => acc.concat(v), []);
						}
					}
				}

				function getIssnValues() {
					return fields
						.filter(field => {
							const dcPath = getDCPath(field);
							return dcPath === 'dc.relation.issn' && field.$.value;
						})
						.map(f => f.$.value);
				}
			}
			...
			function generate500and502() {
				...
				function generate500() {
					...
					function generateLevel() {
						const value = getFieldValues('dc.type.ontasot')[0];
				...
				function generate502() {
					...
					function generate502Subfields() {
						...
						const issueDate = generateIssueDate();
						const subfields = [{code: 'a', value: 'Väitöskirja :'}];

						if (organizations.length > 0) {
							const facultySubfield = generateFacultySubfield();

							if (facultySubfield) {
								return subfields.concat(facultySubfield, issueDate);
							}

							return subfields.concat(generateOrganization(), issueDate);
						}
		...
		function generate506() {
			const staticFields = generateStaticFields();
			const accessFields = generateAccessFields();

			return staticFields.concat(accessFields);
		...
		function generate540() {
			...
			return rights.concat(uri, url, staticFields);
			...
			function generateRights() {
				...
				return values.map(value => {
					...
					function generateSubfields() {
						return /All rights reserved/.test(value) ? [{code: 'a', value}] : [{code: 'c', value}];
					}
				});
			}
		...
		function generate856() {
			const legalDepositFields = generateLegalDeposit();
			...
			function generateLinkSubfields() {
				...
				function generateUrn() {
					const urn = getFieldValues('dc.identifier.urn');
					return urn.length > 0 ? [{code: 'u', value: `${urnResolverUrl}/${urn[0]}`}] : [];
				}

				function generateDoi() {
					const values = getFieldValues('dc.identifier.doi');
					return values.length > 0 ? [{code: 'u', value: values[0]}] : [];
				}
				...
				function generateUrl() {
					const values = getFieldValues('dc.identifier.url');
					return values.length > 0 ? [{code: 'u', value: values[0]}] : [];
				}
			}
