...
export default async function (stream) {
	...
	function convertRecord(record) {
			...
			if( conf.marcTag === '008'){ //Controller fields
				...
				if(foundRec && conf.unique){
					...
					if( orderArr && foundRec.subfields.length >= 1){
						// console.log("-------------- Push new field in order --------------");
						...
						// console.log("In array: ", foundRec.subfields, ", insert new subfield with code: ", conf.marcSub, " to position: ", indexPos);
						foundRec.subfields.splice(indexPos, 0, {
							code: conf.marcSub,
							value: valueFixing(conf, field)
						})
						// console.log("End result: ", foundRec.subfields);
						// console.log("---------------------------")
		...
		function generateControlFields(){
			...
			// console.log("Final control field: ", controlField008);
		...
		function generateMarcRecord(){
			marcJSON.forEach(field => {
				...
				// //ToDo: remove this if proven to work
				// if(orderMap.get(field.tag) && field.subfields.length > 1){
				// 	console.log("--------- This (", field.tag, ") should be sorted in order: ", orderMap.get(field.tag), "--------------");
